...
conf.training_type = 'Block';             % Options: 'Block_Single', 'Block', 'Comb'
conf.nb_subcarriers = 8196;      %number of subcarriers
conf.OFDM_symbs_per_frame = 20;  %number of OFDM data symbols
conf.nb_symbs_between_training = 20;  % number of symbols between each training of the signal
conf.comb_insertion_rate = 4;         % number of QPSK data symbols between each training symbol in comb
conf.comb_symbs_between_training = 4;           % rate at wich we insert the training symbols into OFDM data symbols 
...
conf.OFDM_frame = 0;             %OFDM frame (1 OFDM training symbol + 1 OFDM data symbol)
conf.training_comb = 0;

%Number of data QPSK symbols transmitted per frame: in each time frame T we transmit
%1 OFDM symbol. Each OFDM symbol comprises nb_subcarriers QPSK symbols
%transmitted in parallel at different carrier frequencies. The number of
%QPSK data symbols is the nb_subcarriers time the num of OFDM data symbols.
...
conf.f_spacing = 0.5;     % 1/T [Hz]. T = duration of OFDM symbol
...
% OS factor of our system. It will feed OSIFFT and OSFFT.
conf.os_factor = ceil(conf.f_s / (conf.f_spacing * conf.nb_subcarriers));
...
% Results
%{
image = imread('matlab_image.jpg');
image = rgb2gray(image);
[n_rows, n_cols, n_ch] = size(image);
pixel_stream = reshape(image, [n_rows * n_ch * n_cols], 1);
bitstream = reshape(dec2bin(pixel_stream, 8).', 1, []).';
bitstream = bitstream - '0';
nbits_image = length(bitstream);
%}
conf.OFDM_symbs_per_frame = ceil((nbits_image/conf.modulation_order) / conf.nb_subcarriers);
conf.nb_symbs_between_training = conf.OFDM_symbs_per_frame;
txbits = zeros(2*conf.OFDM_symbs_per_frame * conf.nb_subcarriers, 1);
txbits(1:nbits_image, 1) = bitstream;
...
%     wavwrite(rawtxsignal,conf.f_s,16,'out.wav')   
    ...
    elseif strcmp(conf.audiosystem,'bypass')
        ...
        %H = 2*exp(1i*(pi/2));
        %rxsignal   = rxsignal;
        %rxsignal   = awgn_channel(rxsignal, 5);
    end
    ...
    rx_image = rxbits(1 : nbits_image, 1);
    res.rxnbits(k)      = length(rx_image);  
    res.biterrors(k)    = sum(rx_image ~= bitstream);
    
   rx_image_m = reshape(rx_image, 8, length(rx_image)/8).';
   reconstructed_image = bi2de(rx_image_m);
   reconstructed_image = uint8(reconstructed_image);
   reconstructed_image = reshape(reconstructed_image, n_cols, n_rows).';
