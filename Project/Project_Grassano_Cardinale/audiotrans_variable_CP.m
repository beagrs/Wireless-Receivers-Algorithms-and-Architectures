...
ber_vec = zeros(length(conf.CP_ratio), 1);


for k=1:length(conf.CP_ratio)
    conf.N_samples_CP = ceil(conf.f_s * conf.CP_ratio(k) / (conf.f_spacing));    % Number of samples of the CP
...
figure
semilogy(conf.CP_ratio, (ber_vec), 'g', 'LineWidth', 2);
grid on;
xlabel('CP Length / Number of Subcarriers');
ylabel('BER')
title('BER vs Ratio of CP length')

figure
efficiency = conf.nb_subcarriers / (conf.nb_subcarriers + conf.N_samples_CP);
semilogy(conf.CP_ratio, (ber_vec), 'g', 'LineWidth', 2);
grid on;
xlabel('Efficiency');
ylabel('BER')
title('BER vs Efficiency')
