...
conf.OFDM_symbs_per_frame = 50;  %number of OFDM data symbols
conf.nb_symbs_between_training_vec = [50, 25, 10, 5, 2, 1];  % number of symbols between each training of the signal
...
conf.nb_training_symbs_vec = ceil(conf.OFDM_symbs_per_frame ./ conf.nb_symbs_between_training_vec);
...
ber_vec = zeros(length(conf.nb_symbs_between_training_vec),1);

for k=1: length(conf.nb_symbs_between_training_vec)
    %% Generate the Transmitted Signal
    conf.nb_symbs_between_training = conf.nb_symbs_between_training_vec(k);
    conf.nb_training_symbs = conf.nb_training_symbs_vec(k);
    % Generate the bits
...
%     wavwrite(rawtxsignal,conf.f_s,16,'out.wav')   
    ...
    ber_vec(k) = sum(res.biterrors)/sum(res.rxnbits);
    
end

per = sum(res.biterrors > 0)/conf.nframes
%ber = sum(res.biterrors)/sum(res.rxnbits)
ratio_vec = conf.nb_training_symbs_vec ./ (conf.nb_training_symbs_vec + conf.OFDM_symbs_per_frame);
figure
semilogy(ratio_vec, ber_vec, 'k.-', 'LineWidth', 2, 'MarkerSize', 20);
grid on;
ylabel('BER')
xlabel('Number of training symbols / Number of total symbols')
title('BER vs Ratio of Training Symbols')
